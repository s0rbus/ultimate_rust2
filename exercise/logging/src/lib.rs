...
impl Frog {
    ...
    pub fn hop(&mut self) {
        ...
        info!(target: "Frog:hop", "A Frog hopped, has {} energy left", self.energy);
        if self.energy == 0 {
            // 4. Use warn!() to warn that the frog will go to sleep since he ran out of energy
            warn!(target: "Frog:hop", "Frog going to sleep having run out of energy");
            self.sleep();
        }
    }
...
impl Default for Frog {
    fn default() -> Self {
        ...
        trace!(target: "Frog:default", "default Frog created: {:?}", frog);
        frog
    }
}
